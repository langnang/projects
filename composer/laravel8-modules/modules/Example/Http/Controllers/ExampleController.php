...
class ExampleController extends \App\Http\Controllers\Controller
{
    ...
    public function view_index(Request $request)
    {
        $return = [
            ...
            // 'Example' => app('files')->directories('modules\\' . Module::currentConfig('packages.name') . '\\' . config('modules.paths.generator.views.path') . '\\' . Module::currentConfig('packages.slug')),
            'Example' => app('files')->directories(module_path(Module::currentConfig('name'), config('modules.paths.generator.views.path') . DIRECTORY_SEPARATOR . Module::currentConfig('packages'))),
        ];
        return self::view($return['view'], $return);
    }
    public function view_Example(Request $request, $path)
    {
        $return = [
            'view' => "example::example-packages.index",
            'Example' => app('files')->directories(module_path(Module::currentConfig('name'), config('modules.paths.generator.views.path') . DIRECTORY_SEPARATOR . Module::currentConfig('packages'))),
            'path' => $path,
            'slug' => explode('/', $path)[0]
        ];
        ...
        if (app('files')->isDirectory($example_view) && app('files')->isFile($example_view . DIRECTORY_SEPARATOR . 'index.blade.php')) {
            ...
            $return['view'] = "example::example-packages." . str_replace([DIRECTORY_SEPARATOR], ["."], $path) . ".index";
        } else if (app('files')->isFile($example_view . '.blade.php')) {
            $return['isFile'] = true;
            $return['view'] = "example::example-packages." . str_replace([DIRECTORY_SEPARATOR], ["."], $path);
        } else {
            abort(404);
        }
        return self::view($return['view'], $return);
