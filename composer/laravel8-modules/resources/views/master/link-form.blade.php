  ...
  <div class="container py-3">
    <form class="card" method="POST">
              ...
              @foreach (\Arr::get($options, 'link.type') ?? [] as $option)
                <div class="col">
                  <div class="form-check">
                    <input class="form-check-input @error('type') is-invalid @enderror" type="radio" name="type" value="{{ $option['value'] }}" @if ($link->type == $option['value']) checked @endif>
          ...
          <div class="col-sm-10">
            <div class="row @error('status') is-invalid @enderror">
              @foreach (\Arr::get($options, 'link.status') ?? [] as $option)
                <div class="col">
                  <div class="form-check">
                    <input class="form-check-input @error('status') is-invalid @enderror" type="radio" name="status" value="{{ $option['value'] }}" @if ($link->status == $option['value']) checked @endif>
                    <label class="form-check-label">
                      {{ $option['name'] }}
                    </label>
                  </div>
            ...
            <select class="form-control" name="parent">
              ...
              @isset($links)
                @foreach ($links as $_link)
                  <option value="{{ $_link->id }}" class="d-flex align-items-center" @if ($_link->id == $link->parent) selected @endif>
                    {{ $_link->status }} |
                    {{ $_link->type }} |
                    {{ $_link->title }}
                  </option>
      ...
      <div class="card-footer d-flex justify-content-end">
        <a type="submit" class="btn btn-danger mr-auto" href="{{ url((isset($module) ? $module['alias'] . '/' : 'home/') . 'delete-link/' . $link->id) }}" onclick="event.preventDefault();document.getElementById('destroy-form').submit();">destroy</a>
    ...
    <form id="destroy-form" action="{{ url((isset($module) ? $module['alias'] . '/' : 'home/') . 'delete-link/' . $link->id) }}" method="POST" class="d-none">
      @csrf
    </form>
  </div>
@endsection
