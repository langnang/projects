    ...
    <ul class="list-group list-group-flush">
      @foreach ($children ?? [] as $meta)
        <a class="list-group-item text-truncate py-1 pr-1"
          href="{{ url((isset($module) ? $module['alias'] . '/' : '') . 'meta/' . $meta->mid) }}"
    ...
    <div class="card-body py-2 pl-2 pr-1">
      @foreach ($metas['tags'] ?? [] as $meta)
        <a class="badge badge-pill badge-primary text-truncate my-1"
          href="{{ url((isset($module) ? $module['alias'] . '/' : '') . 'meta/' . $meta->mid) }}"
          title="{{ $meta->name }}" style="max-width: 100%;">{{ $meta->name }}</a>
      @endforeach
    </div>
  </div>
@endempty
  ...
  <div class="card my-2">
    ...
    <ul class="list-group list-group-flush">
      @foreach ($contents['latest'] ?? [] as $content)
        <a class="list-group-item text-truncate py-1  pr-1"
          href="{{ url((isset($module) ? $module['alias'] . '/' : '') . 'content/' . $content->cid) }}"
  ...
  <div class="card my-2">
    ...
    <ul class="list-group list-group-flush">
      @foreach ($comments['latest'] ?? [] as $comment)
        <a class="list-group-item text-truncate py-1 pr-1"
          href="{{ url((isset($module) ? $module['alias'] . '/' : '') . 'content/' . $comment->cid) }}"
